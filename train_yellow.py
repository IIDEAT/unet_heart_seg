...
import os
import sys
from loss.dice_focal_loss import ComboLoss
...
from eval_yellow import eval_net
from unet import UNet
from loss.focalloss import focal
from torch.utils.tensorboard import SummaryWriter
from utils.dataset_yellow import BasicDataset
from torch.utils.data import DataLoader, random_split

dir_img_train = './data/imgs/train/'
dir_mask_train = './data/masks_yellow/train/'
dir_img_val = './data/imgs/val/'
dir_mask_val = './data/masks_yellow/val/'
dir_checkpoint = './logs/'
np.random.seed(20)
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='bmp') as pbar:
            for batch in train_loader:
                ...
                wei = {
                    "weights": {"bce": 1, "focal": 1, "dice": 1},
                    "channel_weights": [1, 1, 1],
                    "channel_losses": [["bce", "focal", "dice"], ["bce", "focal", "dice"], ["bce", "focal", "dice"]],
                    "per_image": False
                }
                loss_function = ComboLoss(**wei)
        ...
        if save_cp:
            ...
            torch.save(net.state_dict(),
                       dir_checkpoint + f'latest_yallow.pth')
            print(val_score, max_score)
            if val_score > max_score:
                max_score = val_score
                torch.save(net.state_dict(),
                           dir_checkpoint + f'best_yallow.pth')
...
def get_args():
    ...
    parser.add_argument('-b', '--batch-size', metavar='B', type=int, nargs='?', default=18,
